...
# This is a basic script to decommission "nodes" in Hbase w/ HDFS over
...
# hadoop-hdfs-over-lustre-or-hdfs-over-networkfs-nodes-decommission-script.sh
...
if [ "${nodecounttodecommission}" -ge "${HBASE_REGIONSERVER_COUNT}" ]
then
    echo "Cannot decommission more nodes than are available"
    exit 1
fi

nodesleft=`expr ${HBASE_REGIONSERVER_COUNT} - ${nodecounttodecommission}`
