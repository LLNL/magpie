...
then
    ...
    then
        ...
        if [ $hiveneedspathupdate != "0" ]; then
            listofdbs=`${2}/bin/beeline -u jdbc:hive2://${HIVE_MASTER_NODE}:${HIVE_PORT} -e 'show databases;'|grep '| ' | awk '{print $2}' `
            for db in `echo $listofdbs`; do
                # filter out the 'database_name' header
                if [[ "$db" != "database_name" ]]; then
                    tabledata=`${2}/bin/beeline -u jdbc:hive2://${HIVE_MASTER_NODE}:${HIVE_PORT}/${db} -e 'show tables;' |grep '| ' | awk '{print $2}'`
  
                    for table in `echo $tabledata`; do 
                        # this filters out the 'tab_name' header in the sql response
                        if [[ "$table" != "tab_name" ]]; then
                            echo "describe formatted ${table};" >> ${HIVE_LOCAL_SCRATCHSPACE_DIR}/${db}_descr_formatted_tables.sql
                        fi 
                    done

                    # We need to grab the path too, possible that they're located at a different location on a different server?
                    tablepaths=`${2}/bin/beeline -u jdbc:hive2://${HIVE_MASTER_NODE}:${HIVE_PORT}/${db} -f ${HIVE_LOCAL_SCRATCHSPACE_DIR}/${db}_descr_formatted_tables.sql |grep 'path'`
                    ...
                    if [ -n "$tablepaths" ]; then
                        for path in `echo $tablepaths`; do
                            ...
                            if [[ $path = *"hdfs://"* ]]; then
                                ...
                                echo "ALTER TABLE ${curtable} SET SERDEPROPERTIES ('path'='${newpath}');" >> ${HIVE_LOCAL_SCRATCHSPACE_DIR}/${db}_update_hive_path.sql
                            fi
                        done

                        ${2}/bin/beeline -u jdbc:hive2://${HIVE_MASTER_NODE}:${HIVE_PORT}/${db} -f ${HIVE_LOCAL_SCRATCHSPACE_DIR}/${db}_update_hive_path.sql
                    fi
                fi    
            done

            #remove both sql files
            rm ${HIVE_LOCAL_SCRATCHSPACE_DIR}/*_descr_formatted_tables.sql
            rm ${HIVE_LOCAL_SCRATCHSPACE_DIR}/*_update_hive_path.sql
        fi
    fi
else
