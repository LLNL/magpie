...
then
    ...
    then
        ...
        if [[ $postgrescheck != 0 ]] 
        then
          echo "pg_isready reports: ${postgrescheck} -- Postgres not ready on ${PGPORT}.  Exiting Hive."
          exit 1
        fi
        ...
        # If the current node is different from the node where metastore is running the JDBC connector throws errors 
        # need to make sure $HIVE_HOME/bin is in path, then call:
        # $HIVE_HOME/bin/metatool -listFSRoot  <-- this will show where the metastore is now
        # if that is different from the current node, run:
        # $HIVE_HOME/bin/metatool -updateLocation <new location> <old location>  <-- this updates the metastore to the new node
    ...
    then
        ...
        if [ $hiveneedspathupdate != "0" ]; then
            tabledata=`${2}/bin/beeline -u jdbc:hive2://localhost:10000/default -e 'show tables;' |grep '| ' | awk '{print $2}'`
            ...
            tablepaths=`${2}/bin/beeline -u jdbc:hive2://localhost:10000/default -f ${HIVE_LOCAL_SCRATCHSPACE_DIR}/descr_formatted_tables.sql |grep 'path'`
            ...
            if [ -n "$tablepaths" ]; then
              ...
              ${2}/bin/beeline -u jdbc:hive2://localhost:10000/default -f ${HIVE_LOCAL_SCRATCHSPACE_DIR}/update_hive_path.sql
            fi
